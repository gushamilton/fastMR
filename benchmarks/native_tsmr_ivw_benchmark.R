...
run_native <- function(g, ne, no, params) {
  out <- vector("list", ne * no)
  k <- 0L
  for (i in seq_len(ne)) for (j in seq_len(no)) {
    k <- k + 1L
    z <- data.frame(
      SNP = d[["SNP"]], beta.exposure = g$exposure_beta[i, ],
    ...
    out[[k]] <- suppressMessages(suppressWarnings(
      TwoSampleMR::mr(z, method_list = "mr_ivw", parameters = params)))
  }
  do.call(rbind, out)
}
...
for (i in seq_len(nrow(result))) lines <- c(lines, sprintf(
  "| %s | %d | %.3f | %.3f | %.2fx | %.3e | %.3e |",
  result$scenario[i], result$pairs[i], result$fastMR_seconds[i],
  result$TwoSampleMR_seconds[i], result$speedup[i],
  result$max_abs_beta_difference[i], result$max_abs_se_difference[i]))
writeLines(lines, file.path(root, "outputs", "native_tsmr_ivw_benchmark.md"))
